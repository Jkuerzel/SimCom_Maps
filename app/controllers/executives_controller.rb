class ExecutivesController < ApplicationController
  ...
  def create
    ...
    if the_executive.valid?
      the_executive.save
      redirect_to("/maps/#{the_executive.map_id}", { :notice => "Executive created successfully." })
    else
      redirect_to("/maps/#{the_executive.map_id}", { :alert => the_executive.errors.full_messages.to_sentence })
    end
  end
  ...
  def update
    ...
    if the_executive.valid?
      the_executive.save
      redirect_to("/maps/#{the_executive.map_id}", { :notice => "Executive updated successfully."} )
    else
      redirect_to("/maps/#{the_executive.map_id}", { :alert => the_executive.errors.full_messages.to_sentence })
    end
  end
